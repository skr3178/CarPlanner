...
- Each polyline has **3 × N_p** points: lane centerline, left boundary, right boundary — concatenated along the point axis.
- Total per-polyline tensor: N_p × 3D_m.
- Count: N_{m,1} polylines.
- Encoded by a **PointNet over the 3N_p points** → single vector of dim D per polyline.
- Output: N_{m,1} × D.
...
**Architecture:**
1. **Agent encoder** — PointNet over (N × Dₐ) at t=0 → per-agent feats N × D + global feat D
...
- **c_lon**: bin index of mean ego speed: j such that mean_speed ∈ [j/N_lon, (j+1)/N_lon) × MAX_SPEED
...
| 6 | Map structure (§2.3) | Polylines: 3×N_p (center + left + right boundary); Polygons: separate PointNet | Centerline only; no boundaries; no polygons | Medium |
| 7 | Backbone sharing (§4 Stage B) | s₀ encoder shared between mode selector and IVM backbone | s₀ encoder feeds mode selector; IVM re-encodes independently | Low |
| 8 | c_lat assignment (§3.2) | Closest candidate route to GT endpoint (route proximity) | Route-proximity via `_route_lateral_offset` — correct approach, but graph-search routes not enumerated; uses encoded lane centroids instead | Low |
