...
| B (IL) | Mode selector + Policy | L\_CE + L\_SideTask + L\_generator (Eq 6+7+11) |
| C (RL) | Policy + Value | PPO + L\_generator (Eq 8–11) — *deferred* |
...
1. **Causal Confusion:** Imitation learning suffers from distribution shift and causal confusion — learning correlations instead of actual causations.

   > Model sees dashboard + road → When braking, brake light turns ON → Model learns: "Brake when brake light is ON"
...
2. **Multi-modal explosion** — exploration space explodes exponentially
...
- Perform an ablation study with and without Autoregressive IVM
- Mode selector — mode scores: what are the different modes and how are they defined?
- Vanilla vs autoregressive
- Non-reactive component — other cars do not change shape — is this then a partial world model?
- Rule selector
- Design of IVM (Invariant-view module)

---

## Planning Over a Horizon: Short, Medium and Long Horizon Tasks

**Short Horizon:** Pros: very reactive. Cons: needs massive data.

**Long Horizons:** Pros: reactive enough, efficient enough. Cons: less reactive (e.g. child or pedestrian comes in front).

**Optimization objective:**

```
π* = argmax_π  E_{s_t ~ P_τ, a_t ~ π} [ Σ_{t=0}^{T} γ^t R(s_t, a_t) ]
```

**Reward function:**

```
R(s_t, a_t) =   w1 * forward_progress
...
## Change Standard RL Formulation to Autoregressive Type

Leads to faster simulation and efficient training.

**Before:** `s_{t+1} ~ P(s_{t+1} | s_t, a_t)`

**After (match trick):** `s_{t+1} ~ P_hat(s_{t+1} | s_t, a_t)`

**Advantages:**

- Simulate multiple trajectories
- GPU-based
- Without real environment

---

## Planner Architecture Components

1. **Nonreactive transition model** — predicts future agent positions
2. **Mode selector** — scores all possible driving modes
3. **Trajectory generator** = ModeSelector + AutoregressivePolicy — predicts ego trajectory
4. **Rule-augmented selector** — inference-only, picks best trajectory from 60 candidates using rules (no learning)

Initial state (`s_0`), possible `N_modes`. Trajectory selector evaluates and assigns score to each mode. Creating `N_mode` parallel worlds. Policy rollout — trajectory predictor acts as state transition model, generating future poses of traffic agents across time horizons.
...
- **Quality rewards:** collision rate, drivable area compliance
...
**Why CarPlanner references PDM (Policy Driven Motion Planning):**

- Define a policy over a set of candidate trajectories — score and select best one
- Trajectory scoring: safety, comfort, progress, rule compliance
...
- Distribution shift
- Causal confusion
- Cannot exceed expert performance and generalize
...
| 7 | box\_l | box.length | ❌ |
| 8 | box\_h | box.height | ❌ |
| 9 | time\_step | relative timestamp | ❌ |
| 10 | category | tracked\_object\_type | ❌ |

We currently only use 4 of 10. Missing: vx, vy, box dims, time\_step, category.

### Map Point Features (Dm=9)

The paper says x, y, heading, speed\_limit, category per point, but that's 5 not 9. The most likely 9-dim encoding:

| # | Feature | Source |
|---|---------|--------|
| 1-2 | x, y | `discrete_path[i].x/.y` |
| 3-4 | sin(heading), cos(heading) | `discrete_path[i].heading` |
| 5 | speed\_limit | `lane.speed_limit_mps` |
| 6-9 | category one-hot | LANE / LANE\_CONNECTOR / CROSSWALK / STOP\_LINE |
...
**Paper:** Loop over D, run β(s\_0), compute L\_tm (Eq 5), backprop, update β, then freeze.
...
**Paper:** Use π, s\_0, c\*, s^{1:N}\_{1:T} → collect a\_{0:T-1} → stack → L1Loss vs GT.
...
#### Step 2, lines 36–40 — π\_old update every I steps
...
| Alg 1 line 16 — c\_lat from s\_0/route | No | c\_lat from GT endpoint y-offset |
| Alg 1 line 20 — f\_selector(s\_0, c) | No | c not passed to selector |
| Alg 1 lines 28–31 — IL L\_gen | ✓ | |
| Alg 1 lines 33–35 — total loss | ✓ | |
| Alg 2 step 1 — s\_0 encoding | Partial | No global Transformer |
...
**Most impactful gaps for IL quality:** c\_lat from route (not GT), map not re-transformed per IVM step, and no agent time history in IVM.

---

## IVM (Invariant-View Module)

IVM strips out everything that would make the policy time-dependent — absolute position, absolute time, distant irrelevant agents — so the same learned policy generalises across all timesteps in the rollout.

**Techniques:**

1. **K-nearest neighbor filtering** (reduce noise) — removes irrelevant context, focuses attention on what actually matters for the next action.
2. **Ego-centric coordinate transform** (remove absolute position) — the policy never sees global coordinates. It only sees "what's around me right now." This makes it position-invariant.
3. **Time normalisation** (remove absolute time) — the policy always sees "the last H steps leading to now" with the same indices. It never knows whether it's at absolute step 3 or step 7.
...
| ego\_pose | 51,152 | AV pose at every IMU sample (~100Hz) |
| lidar\_pc | 10,200 | LiDAR sweep timestamps + file refs (10Hz) |
| lidar\_box | 1,280,922 | Detected bounding boxes per LiDAR sweep |
| track | 6,460 | Unique tracked objects (vehicle/ped/bike dims) |
| scenario\_tag | 13,812 | Labels marking which frames belong to scenario types |
| traffic\_light\_status | 96,994 | Red/green per lane connector per frame |
| scene | 26 | Scene-level goal poses + roadblock IDs |
| category | 7 | Object class definitions |

### ego\_pose columns and ranges

| Column | Range | Unit |
|---|---|---|
| x, y | [664,425 → 664,656], [3,996,370 → 3,999,268] | m (global UTM) |
| vx, vy | [-0.02 → 14.05], [-0.32 → 0.04] | m/s |
| acceleration\_x | [-1.77 → 3.07] | m/s² |
| qw/qx/qy/qz | quaternion | — |
| angular\_rate\_x/y/z | — | rad/s |

### lidar\_box columns and ranges (detected agents)
...
| traffic\_cone | Temporary cones |
| barrier | Permanent/temporary barriers |
| czone\_sign | Construction zone signs |
| generic\_object | Animals, debris, poles |

### scenario\_tag types (frequency)

| Type | Count |
|---|---|
| stationary | 3,020 |
| stationary\_in\_traffic | 2,034 |
| traversing\_intersection | 1,667 |
| on\_traffic\_light\_intersection | 1,291 |
| high\_magnitude\_speed | 1,122 |
| near\_pedestrian\_on\_crosswalk | 250 |
| traversing\_crosswalk | 65 |
| ... | ... |
...
| Epoch | L\_total | L\_CE | L\_side | L\_gen | Time |
|---|---|---|---|---|---|
...
|---|---|---|
| L\_CE | Mode classifier | Can't identify the right behavioral intention |
| L\_side | Mode selector's trajectory sketch | Mode choice doesn't match trajectory shape |
| L\_gen | Autoregressive policy rollout | Policy can't predict next waypoints accurately |
...
|---|---|---|
| `agents_now` | `(B, N=20, Da=10)` | Agent states at t=0 in ego frame: x, y, heading, vx, vy, box\_w, box\_l, box\_h, time\_step, category |
...
| Symbol | Name | Meaning |
|---|---|---|
| `D` | Dataset | nuPlan scenarios, each a tuple of initial state + GT trajectories |
| `s_0` | Initial state | Scene at t=0: ego pose, all agent states, map — the full observation |
| `s^{0:N,gt}_{1:T}` | GT trajectories | Ground-truth future states for agents 0..N across timesteps 1..T. Superscript = agent index (0=ego, 1..N=others). Subscript = time range |
| `c_lon` | Longitudinal mode label | Pre-computed speed bin from GT — which of 12 speed intervals the ego lands in |
| `γ` | Discount factor | RL reward discounting (=0.1 per paper) |
| `λ` | GAE parameter | GAE advantage estimation smoothing (=0.9 per paper) |
| `I` | Update interval | How many steps between copying π → π\_old (PPO delayed update) |
| `β` | Transition model | The non-reactive model that predicts agent futures given s\_0 |
| `f_selector` | Mode selector | Predicts mode scores σ and side-task trajectory s̄ |
| `π` | Policy (current) | Autoregressive trajectory generator, updated each step |
| `π_old` | Policy (old) | Frozen copy of π used for PPO importance ratio — updated every I steps |
...
| `s^{1:N,gt}_{1:T}` | GT agent futures — same shape, from dataset |
| `L_tm` | Transition model loss — L1 between predicted and GT agent states (Eq 5) |

### Stage 2 (lines 9–41) — Selector + Generator Training

| Symbol | Meaning |
|---|---|
| `s^{0,gt}_{1:T}` | GT **ego** trajectory (agent index 0 = ego), timesteps 1..T |
| `s^{1:N}_{1:T}` | Agent futures from β (now frozen) — used as the simulated world during rollout |
| `c_lat` | Lateral mode — which of 5 lateral bins (determined from route proximity to s\_0) |
| `c` | Full mode = concatenate(c\_lat, c\_lon) → one of 60 modes |
| `c*` | Positive mode — the single GT-assigned mode label used for CE loss supervision |
| `σ` | Mode logits — (N\_modes,) scores output by f\_selector |
| `s̄^0_{1:T}` | Side-task trajectory — rough ego trajectory predicted by f\_selector conditioned on c |
| `L_selector` | Selector loss = CE(σ, c\*) + SideTaskLoss(s̄, s\_gt) |
...
| `R̂_{0:T-1}` | GAE returns (discounted reward targets for value loss) |
| `d_{0:T-1,new}` | New policy distribution from updated π (used for PPO clipped ratio) |
| `V_{0:T-1,new}` | New value estimates from updated π |
| `L_generator` (RL) | = ValueLoss + PolicyLoss − Entropy |
...
| `L_generator` (IL) | = L1(s^0\_{1:T}, s^{0,gt}\_{1:T}) — Eq 11 |
...
- `s^{1:N}_{1:T}` from β vs `s^{0,gt}_{1:T}` from dataset — same shape structure, different agent/source
...
**The value of `c*`:** A single integer pointing to one cell in the 4×12 grid.

---

## A2C vs CarPlanner (PPO + GAE)

CarPlanner uses the Advantage Actor-Critic in a revised form.

**Biggest difference — Stability.** The problem with vanilla A2C: nothing stops the policy from updating too aggressively on a lucky batch, which can cause catastrophic forgetting or collapse.
...
- PPO+GAE is best understood as A2C with two important engineering improvements: better advantage estimates (GAE) and a safety constraint on how much the policy can change per update (clipping).
- Those two additions are what make it practical for large-scale real-world training.

Reference of earlier implementation of Hierarchical Actor Critic:
`https://github.com/skr3178/reinforcement_learning/blob/main/HAC/README.md`

![explanations](paper/claude_explainations/c_star_intersection.svg)
...
- `agents_future` = `(T_future, N_agents, features)` — predicted future trajectory for every other agent
- `L_tm = L1(pred, gt)`; backprop, update β
- Ego plan + predicted traffic → simulation without simulator

β is frozen for subsequent steps as it determines the future world; it becomes unstable if trained with RL.

### 2. f\_selector — the Mode Selector

Given the initial scene, it scores all possible driving modes (e.g. "turn left slowly", "go straight fast"). It answers: *which of these modes is most appropriate right now?* Trained with supervised learning using the ground truth trajectory to identify the correct mode.
...
### Different losses for different components
...
2. **`L_selector` → f\_selector:** The selector has to answer two questions simultaneously — which mode is correct (classification), and can it also roughly predict the ego trajectory (regression). CrossEntropy handles the classification part. The L1 side task is a bonus signal that forces the selector's internal features to be rich enough to reason about the future, not just assign labels.

   ```
   CrossEntropy  →  trains selector to pick the RIGHT mode
   SideTaskLoss  →  trains selector to UNDERSTAND the scene deeply

   SideTaskLoss = (1/T) Σ_{t=1..8} | ego_pred[t] - ego_gt[t] |
   where:
     ego_pred[t] = [x, y, yaw]   ← what selector thinks ego will do
     ego_gt[t]   = [x, y, yaw]   ← what expert actually did
   ```

3. **`L_generator` → π:** This is where IL and RL diverge. In IL, the generator is told "copy the expert" — L1 between predicted and GT waypoints. In RL, the generator is told "maximise reward" — PPO loss which includes the policy gradient term, a value regression term, and an entropy bonus to prevent the policy from collapsing to a single deterministic action too early.

### Weight of entropy

| Term | Magnitude | Weight | Contribution |
|---|---|---|---|
| ValueLoss | ~10³ | 3 | ~3,000 |
| PolicyLoss | ~10⁰ | 100 | ~100 |
| Entropy | ~10⁻³ | 0.001 | ~tiny |

Entropy of a Gaussian distribution: `H(X) = ½ ln(2πeσ²)`, dependent on the variance/spread.

- High σ → wide distribution → high entropy → policy is uncertain
- Low σ → narrow distribution → low entropy → policy is confident

The minus sign on entropy: minimising `L_gen` means maximising entropy. This is deliberate — it is called an **entropy bonus** and prevents the policy from collapsing too early.

**ValueLoss dominates** because accurate value estimation is the foundation of PPO. If V is wrong, the advantage estimates A\_t are wrong, and the policy gradient signal is meaningless noise. You need the critic to be well-calibrated before the actor can improve.

**PolicyLoss is secondary** because the actual policy update should be conservative — PPO is designed to take small, stable steps. A contribution of ~100 versus ~3,000 for value means the policy updates are restrained relative to critic updates.

**Entropy is a whisper** because it is purely a regulariser. It only matters when ValueLoss and PolicyLoss are both near zero — i.e. when the policy is already good and in danger of collapsing.

---

## Root-Cause Analysis: Stage A Training Issues
...
The trained model never even reached the copy-s\_0 baseline. It started at ~50 (close to predict-zeros), descended, and got stuck at 15.35 — *above* 14.44.

### Finding 2: Raw, unnormalized features with huge dynamic range

Per-feature stats of `agents_seq` (valid agents only):

| Dim | Feature | abs\_mean | std | range |
|---|---|---|---|---|
...
| 5 | box\_w | 1.71 | 0.76 | [0, 5.8] |
| 6 | box\_l | 3.78 | 2.71 | [0, 20.0] |
| 7 | box\_h | 1.78 | 0.66 | [0, 9.1] |
| 8 | **time\_step** | **0.00** | **0.00** | **[0, 0]** |
| 9 | category | 0.93 | 1.56 | [0, 4] |

**Dim 8 (time\_step) is dead — always zero.** Bug in `data_loader.py` extraction.

### Root causes (ranked)

**Cause A: Model has no residual/copy inductive bias — primary.** The architecture max-pools over the H=10 history frames (`model.py:1059`), collapsing all temporal information into one per-agent vector. The decoder has to reconstruct `s_0` from scratch. There's no skip connection from `s_0` to the output.
...
**Cause B: Raw unnormalized features — contributing.** x has mean-abs 29.6m, y has 12.2m. Gradients dominated by far-away agents and box dims. Fix: normalize per-feature to unit std.

**Cause C: LR collapse (confirmed) — secondary.** At checkpoint: `lr = 6.56e-09` (8 reductions from 1e-4). `patience=0` applied to noisy training loss instead of smooth validation loss.

**Cause D: PointNet max-pool over time destroys ordering — architectural.** `max(dim=1)` over H frames doesn't distinguish "moving right" from "moving left."

### Fix priority order

1. **Add residual connection** in `TransitionModel.forward` — `pred = s_0 + delta`
2. **Validation-based scheduler** — use val\_loss for `scheduler.step()`
3. **Normalize features** — divide x/y by ~30m before feeding in
4. **Fix `time_step` extraction bug** in `data_loader.py`
...
Both tests pass: the transition model beats the stationary baseline by 25% and correctly ignores ego perturbations.

---

## Model Component Shapes
...
| AutoregressivePolicy | agents\_seq(2,8,20,10) | (2,8,3) |
| CarPlanner.forward\_train | full inputs | logits(2,60) side(2,8,3) pred(2,8,3) |
...
### Polygon encoder comparison

| Aspect | Option 3 (derived from cache) | Option 2 (real nuPlan polygons) |
|---|---|---|
| Geometry | Lane centerlines repurposed as "polygons" — open polylines, not closed shapes | Real closed polygons from GeoPackage — crosswalks, stop lines, intersections |
| Categories | Only cat0=LANE (all derived from lanes) — no diversity | 3 distinct categories: crosswalks (17%), stop\_polygons (33%), intersections (50%) |
| Spatial distribution | x: [-75, 133]m, y: [-117, 143]m — large area | x: [-21, 42]m, y: [-4, 25]m — tighter, more local |
| Valid per sample | 3.1 avg | 6.0 avg |
| Semantic meaning | None — lanes treated as polygons don't represent crosswalks/stop lines | Correct — intersections, crosswalks, stop lines |
| Speed limit | Non-zero (1.18 std) — inherited from lanes | Always 0 — polygons don't have speed limits (correct) |

**Bottom line:** Option 3 only validates that the architecture works on real tensor shapes. Option 2 validates the architecture on actual polygon geometry with correct semantics and category diversity. Option 2 is the right choice.

### Data loader additions needed
...
Each polygon needs to be resampled to Np points and encoded into its own tensor `(N_POLY, N_PTS, D_POLY_POINT)` — similar to lanes but for closed polygons rather than open polylines.

---

## Stage B Speedup Analysis

β is already pre-computed and cached on CPU. The remaining cost is the **autoregressive policy loop in `model.py` (~lines 667–794), which runs T\_FUTURE=8 times per batch**.
...
bf16 is safer than fp16 on 40-series GPUs — no loss scaling, no NaN risk from the L1 losses.

**2. Vectorize the H=10 frame-transform Python loop (line 688)** — expect 1.3–1.8×

Replace Python `for` loop with a single batched matmul: stack the 10 history frames into `(B, N, H, 3)`, build a single `(B, 2, 2)` rotation matrix per step, and apply with `einsum`.

**3. Cache LaneEncoder output once per sample** — expect 1.2–1.4×

Encode lanes once in the step-0 ego frame, then for each step pass the ego-delta `(Δx, Δy, Δθ)` as an extra key to IVM.
...
**8. Reduce logging/checkpoint frequency** — already fine.

### What NOT to do

- **Don't drop to GT `agents_seq`** — breaks paper faithfulness
- **Don't skip the IVM 3-layer stack** — that's a paper hyperparameter
- **Don't use fp16** — L1 losses + long autoregressive chain are fp16-hostile; bf16 is the right call

### Recommended order

1. Add bf16 autocast → restart. Measure new epoch time.
2. Vectorize frame transforms.
3. Bump batch size + pin β.
4. LaneEncoder caching.
5. Last resort: `torch.compile`.

**Expected cumulative speedup: ~3–4×**
...
- Stage C asks "can we improve on what humans do?" by rewarding safety, staying on-road, and smoothness — using RL to fine-tune without forgetting Stage B's knowledge

The `L_gen` + `L_selector` losses in Stage C act as a leash to prevent RL from wandering too far from Stage B's behavior.

**Stage C inputs:**

1. **Stage A checkpoint** (`stage_a_best.pt`) — the transition model β. Frozen — Stage C never updates it, just uses it to simulate the scene.
2. **Stage B checkpoint** (`best.pt`) — the policy π and mode selector, used as the warm start. Stage C inherits all of Stage B's learned IL behavior and fine-tunes it with RL.
...
```bash
# 1. Stage A
nohup python -u train_stage_a.py --split mini --epochs 50 --batch_size 1024 \
  > checkpoints/stage_a_train.log 2>&1 &
...
**What the model does:** Given the current scene, it generates **N\_MODES trajectories** — one per driving mode. Each is a candidate "plan" for what the ego should do next.

**Best-of-K:** Of all the modes the model generated, how close is the best one to what the human actually did?
...
- **Best-of-K** measures the model's *coverage* — can it generate a trajectory close to GT?
- **Mode accuracy** measures whether the selector correctly identifies *which* mode matches GT
- **Limitation:** This is open-loop — it doesn't simulate what happens if the car follows the predicted path
...
1. **CarPlannerFeatureBuilder** — At each simulation step, converts:
   ...
   - All coordinates transformed to ego-centric frame

2. **CarPlannerPlanner** — Wraps the model:
...
3. **eval\_nuplan.py** — Launch script:
...
The nuPlan devkit environment must be fully set up (the simulation requires the full devkit stack).

---

## Environment Setup

### Training / open-loop eval venv

```
/media/skr/storage/autoresearch/CarPlanner_Implementation/paper/.venv
```

- Has: torch, CarPlanner model code
- Used for: `train_stage_a/b/c.py`, `evaluate.py`, `preextract_features.py`
- Does NOT have nuPlan simulation deps

### nuPlan closed-loop eval venv

```
/media/skr/storage/autoresearch/CarPlanner_Implementation/paper/dataset/nuplan-devkit/nuplan_venv
```

- Has: full nuPlan devkit deps (pytorch\_lightning 1.3.8, tensorboard, ray, shapely, etc.)
